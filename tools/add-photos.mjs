...
function watermarkSvg(w, h) {
  /* keep in sync with tools/rewatermark.mjs: corner mark only, low opacity */
  const small = Math.round(w / 58);
  return Buffer.from(
    `<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">` +
      `<text x="${w - 14}" y="${h - 12}" text-anchor="end" ` +
      `font-family="Georgia, serif" font-size="${small}" ` +
      `fill="black" fill-opacity="0.16" dx="1" dy="1">${WATERMARK}</text>` +
      `<text x="${w - 14}" y="${h - 12}" text-anchor="end" ` +
      `font-family="Georgia, serif" font-size="${small}" ` +
      `fill="white" fill-opacity="0.30">${WATERMARK}</text>` +
      `</svg>`
  );
}
